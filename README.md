...
Uploads dynamic text data to the Swarm network; the data is uploaded to the feed of a given topic.

**Parameters:**

- `data`: String data to upload
- `memoryTopic`: Uploads the data to a Swarm Feed with this topic (requires `BEE_FEED_PK` in config)
...
- `topicString`, `topic`, `feedUrl`: Feed details

### `upload_static_text`

Uploads static text data to the Swarm network.
